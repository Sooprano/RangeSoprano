...
48. Rework "Rangos": banco real (29 3bet lineal por % + 15 3bet GTO Wizard con peso + 5 open RFI + 5 call BB + 5 cold-call BTN/SB = 59 spots, familias `linear-3bet`/`gto-3bet`/`open`/`call`/`cold-call`) · `RangeSpot.family` + scopes por drill · split en 3 tabs ("% y combos" · "Composición" mini-charts · "Tipo de rango" activo con open lineal + call/cold-call condensado) · `MiniRangeChart` (opacidad=peso) · morfología reforzada (mergeado SUBE / condensado PAGA) · **filtros de familia** `ChipFilter` por drill ("Rangos a estudiar") · **demo por defecto reemplazado** (`sampleRanges.ts`): 30 rangos GTO reales en carpeta `Rangos GTO cash` + 4 subcarpetas (OR/3bet/BBDefend/Call cold), reusando las notaciones de los raw files via `parseHandRange`.
