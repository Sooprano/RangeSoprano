...
Fases 1-19 completadas ✅. Última iteración: **fix SEO de indexación en GSC** (commit `04ac4e6`). Sitemap listaba las 3 SPA routes sin trailing slash mientras GitHub Pages las sirve con barra (vía pre-render loop), generando "Error de redirección" en `/viewer` y duplicate indexing en `/editor`+`/trainer`. Sumado, el `<link rel=canonical>` estático en `index.html` apuntaba a `/` para todas las rutas. Fix: trailing slash en `public/sitemap.xml` + `useDocumentTitle` extendido con opción `canonical?: string` que setea el `<link>` en runtime per-route. ViewerPage/TrainerPage/EditorPage pasan su URL canónica self-referente. Tras el deploy, en GSC el usuario re-envió el sitemap, dejó correr la validación pendiente del Error de redirección, y solicitó indexación manual de las 3 URLs con barra. Las versiones sin barra que ya estaban indexadas se consolidan solas en 2-4 semanas porque ahora redirigen a páginas con canonical self-referente coherente.

Iteración previa: **pulido visual de prompts pedagógicos** en Push/Fold Quiz y Pot Odds (Estudio + Velocidad). Push/Fold Quiz pasa de HandBadge textual a dos cartas reales con palos (♠♥/♣♣/♥♠ misma convención que Clásico) más StackChip dedicado con `X.X BB` en amber-200 bold 2xl. Pot Odds reescribe el prompt como ScenarioChip (label muted + valor amber bold) + pregunta corta con keyword en accent-light, y agrega un MiniPot tipo mesa real online: pot stack 100 BB anclado al centro, bet stack proporcional con fichas apiladas y disambiguation espacial — villano a la izquierda, hero a la derecha. Auto-avance ON por default en ambos Estudios (Pot Odds y Push/Fold) con CountdownBar morada animada bajo el feedback (componente extraído de ClassicTrainer y compartido entre los 3 trainers).
