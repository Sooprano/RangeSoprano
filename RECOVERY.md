...
Fases 1-18c completadas ✅. Última feature: Push/Fold trainer (Nash HU para stacks cortos), 5ª pestaña en `/trainer` range-independent con sub-toggle Estudio/Velocidad. Estudio incluye Tabla con slider de stack 1-20 BB que pinta el grid dinámicamente (verde push BTN, azul call BB) + Quiz binario con atajos 1/2/N. Velocidad con duraciones 30/60/120s y leaderboard persistente top 5 por duración. Datos canónicos transcritos de la imagen del usuario en formato row-major idéntico al chart fuente. Cero deps nuevas, +43KB raw / +10KB gzip al chunk de TrainerPage en total para las 3 sub-fases.
...
**Push/Fold Quiz: cartas visuales en lugar de notación textual** — hoy el `HandBadge` muestra solo la notación tipo "Q2s", "AKo", "TT". Sería más inmersivo (especialmente para estudiantes nuevos) renderizar dos cartas reales con palos visibles (♠♥♦♣) y rank grande, estilo de mesa de poker clásica. Implementación: para `XYs` elegir 2 palos iguales (ej. ♠♠), para `XYo` elegir 2 palos distintos (ej. ♠♥), para pares `XX` dos palos cualquiera distintos (ej. ♠♥). Render con SVG inline o caracteres Unicode + Tailwind. Aplicaría también al `SpeedPrompt` de PushFoldSpeed para coherencia. Cuidado con el espacio vertical en mobile (las cartas son más altas que el badge actual).

**Push/Fold export/import del leaderboard JSON** — espejo de lo que existe para Pot Odds (fase 16e). Falta: agregar `zPushFoldLeaderboardExportPayload` en `schemas.ts`, exponer botones Download/Upload en el componente `Leaderboard` de `PushFoldSpeed`, reusar `downloadBlob` / `todayIsoDate` / `MAX_IMPORT_BYTES`. Sería fase 18d ~30 líneas netas.
